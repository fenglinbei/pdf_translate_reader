...
当前交付：统一模型目录、翻译与论文问答适配已实现，并通过本地回归、构建及开发环境实调用。尚未部署生产。基础合成样例用于验证调用和简单行为，不能视为真实论文质量验收。
...
1. 用代表性论文覆盖长文总结、公式解释、证据定位、证据不足和连续追问，比较候选模型的质量、延迟与实际 token 消耗，再确认 QA 默认值。
