...
> 当前开发入口（2026-09-24）：[P1 收尾](qa-agent-stage-1-closeout.md) 与
> [P2 工具协议及模型自主查阅文档](qa-agent-stage-2-plan.md)。
> 本文保留较早的完整产品蓝图；其中强制语义索引的设计不再作为新路径的前置条件。
> 跨论文、视觉等远期能力仍不能按本文推定已经实现，P2 当前只覆盖已授权的当前文档文本。
...
### 4.1 可配置的 MathPix 启动与手动 QA 索引策略

PDF 导入、打开文档和云端同步都不能自动触发 QA 索引重计算。MathPix 解析是否在打开 PDF 后自动启动由用户设置控制，默认开启；用户关闭后只恢复或同步已有缓存，不创建新的 MathPix 上游任务。QA 索引仍必须由用户显式点击。

触发入口：

- 打开 PDF 且“自动启动 MathPix”开启：启动现有 MathPix pipeline；优先恢复本地或云端缓存，仅在没有可复用结果时提交 PDF。
- 用户点击“开始 MathPix 解析”：无论自动启动设置是否关闭，都可以手动启动或刷新现有 MathPix pipeline。
...
- 导入 PDF、打开 PDF 或恢复阅读会话后，先在后台协调本地与云端缓存；缓存恢复和补传本身不能提交新的 MathPix 任务。
- “自动启动 MathPix”默认开启；设置完成加载后，当前 PDF 每次打开最多自动启动一次 pipeline。显式关闭时保持手动按钮可用。
- 如果云端已经有同一 `contentSha256 + MATHPIX_OPTIONS_HASH` 的 completed cache，应自动恢复到 IndexedDB，不需要再次提交上游 MathPix。
- 如果已有 submitted/processing 记录，自动或手动启动都进入继续轮询或查看进度，不重复提交。
...
- `ReaderShell` 在设置完成加载后按用户开关决定是否自动启动 MathPix，避免云端保存的关闭选项尚未读取时抢先产生调用。
- MathPix completed pages 和 full MMD 必须立即同步到私有云缓存；本地先完成、云文档 ID 后到或临时同步失败时，在打开文档和恢复在线后幂等补传。
...
### Step 3: MathPix 可配置自动触发和索引 job 基础设施

改动：

- 新增“自动启动 MathPix”设置并默认为开启；设置关闭时不自动创建上游任务。
- 保留“开始 MathPix 解析”按钮和状态处理，支持用户手动启动或刷新。
- 打开 PDF 时自动恢复本地或云端 completed cache，并对未同步的本地结果做幂等云端补传。
- 只有自动设置开启或用户手动点击时才允许调用现有 MathPix pipeline；缓存协调本身不得提交上游任务。
...
- 默认设置下，上传或打开 PDF 会自动恢复缓存，并在没有缓存时启动一次 MathPix；不会自动触发 QA 索引。
- 用户关闭自动启动后，上传或打开 PDF 只恢复、同步已有缓存，不产生新的 MathPix submit/processing 记录。
- 用户点击“开始 MathPix 解析”后，可以复用已完成云端缓存、续接进行中任务或启动新解析。
