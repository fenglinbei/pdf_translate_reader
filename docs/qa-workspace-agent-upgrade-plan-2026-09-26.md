...
日期：2026-09-26。状态：**M0 已完成；M1 实现范围已冻结、业务代码待实施；M2–M7 仍为计划，尚未启用新产品功能。**

M0 完成补记（2026-09-26）：用户确认当前交互，资源/操作/来源/附件/版本与预算已冻结；固定样本、隔离运行/数据库/标题测量和历史守卫已补齐。见 [M0 完成记录](qa-workspace-agent-m0-closeout-2026-09-26.md)、[已采纳方案与视觉稿](qa-workspace-agent-m0-recommendation-2026-09-26.md)、[冻结协议](qa-workspace-agent-m0-contracts-2026-09-26.md)。下文保留各后续里程碑出口，契约冻结不等于代码、人工验收或上线。

M1 范围补记（2026-09-26）：已形成 [M1 云端访问实现范围冻结](qa-workspace-agent-m1-implementation-freeze-2026-09-26.md)，覆盖 12 类云端资源、五个只读工具、多类来源的流式/原子保存/历史/UI 闭环，以及明确的验收和 M2–M6 边界。本轮只完成范围冻结。
...
**M0 已完成设计冻结；M1 实现范围已冻结、代码待实施；M2–M7 为计划。** M0 的基线、测量与验证工具已完成，运行能力按后续阶段实施。每步交付设计说明、代码/迁移、必要测试、脱敏轨迹、局限和回退指针。

| 里程碑 | 主要交付 | 依赖 | 可验收的出口 |
| --- | --- | --- | --- |
| M0 范围、契约与交互冻结（已完成） | 逐类资源/操作/归属矩阵、操作 Schema、来源/附件格式、会话管理和选文交互稿、固定评测集与基线 | 本文审阅 | 产品选择与技术契约明确；新功能未启用，基线可以重现 |
| M1 云端工作区访问（范围已冻结） | 统一仓储/适配器；文库、组织、笔记/高亮、译文/术语、历史问答的查询/读取/统计/关联；多类来源保存与展示 | M0 | 所有上述云端资源至少跑通一个组合任务；计数准确、可回到来源、跨用户隔离成立 |
...
7. 模型错误、超时或配额紧张时保留临时标题。任务持久化；首版自动重试 0，用户显式重新生成另记预算；这套小型辅助任务机制可以先实现，M6 再统一执行可靠性。
...
- 每批至少有相关单元/数据库测试、全量 CI、浏览器流程、供应商合成问答与人工验收记录。纯文档变化只做文档检查；M0 测量按明确的小额预算执行合成标题调用，其他压力测试使用模拟模型；不为规划重启产品服务。
...
- 状态依次记录：计划 → 契约冻结 → 代码完成 → 本地/CI 通过 → 测试部署 → 人工验收 → 正式部署。本轮任务止于 M0 设计冻结、隔离测量与验证工具，不包含部署。

## 11. 实施前的推荐选择

下表方案已经通过 M0 交互稿评审并冻结；具体协议、限额与失败处理以 M0 冻结文件为准。
...
M0 的 Schema、交互稿和基线已冻结，后续按依赖推进。已确认的设计不在每个实施步骤重复询问；新的不可逆数据或运行策略变化另行明确。
