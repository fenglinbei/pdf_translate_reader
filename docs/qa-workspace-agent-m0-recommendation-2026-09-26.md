# 工作区 Agent M0：已采纳方案与交互冻结 v1

日期：2026-09-26。状态：**M0 已完成设计冻结。用户已确认当前交互；剩余契约、测量与守卫收尾见 [M0 完成记录](qa-workspace-agent-m0-closeout-2026-09-26.md)。**

本轮承接[升级计划](qa-workspace-agent-upgrade-plan-2026-09-26.md)，从 `fc091e69c0343f77743c7296f4a441030849606a` 核对现状。交付已采纳决策、视觉稿、冻结协议、固定评测集及可复跑的离线/隔离/辅助模型基线。生产功能、数据库、版本和服务配置均未改变。
...
- [冻结资源与协议](qa-workspace-agent-m0-contracts-2026-09-26.md)：云端/本地资源矩阵、五类工具、字段白名单、覆盖与游标、来源、选文附件、消息 revision、标题原子条件提交和草稿冲突处理。
...
附件初始硬上限已冻结：最多 4 件，单件选文 4,000、选文合计 12,000、补充上下文合计 4,000 个 UTF-16 单位；问题单独限制 2,000，v2 HTTP body 最大 128 KiB。超限明确提示，不静默截断。token 另计并在主模型上下文预算检查；这是 M3 待实现的契约，不表示现有路由已从 64 KiB 提升。

## 6. M0 完成状态与后续出口

2026-09-26 用户确认：“可以先用这版，交互暂时没发现什么问题，请继续完成剩余的M0任务”。该确认冻结本稿的布局和操作方向；不等同于产品代码人工验收。

| M0 出口 | 当前状态与证据 |
| --- | --- |
| 资源/操作/归属/覆盖矩阵 | 已冻结云端与当前设备逐类边界 |
| 工具、来源、附件、revision、预算 | 冻结 v1；Schema 20 合法 / 25 非法样本可重复验证 |
| 桌面/窄屏交互 | 原型浏览器检查通过；用户已确认 |
| 固定数据与预期 | 38 个案例冻结；仍为 not_executed，供 M1–M7 接入真实功能 |
| 旧执行器守卫 | 原历史 SHA 保留；版本兼容 7/7，严格对照仍 3 一致 / 4 差异，见[溯源记录](qa-workspace-agent-m0-equivalence-2026-09-26.md) |
| 运行与 HTTP/SSE | 2 CPU / 2 GiB、三轮 1,230 次合成请求，冻结队列、缓存、扫描与性能回归口径，见[运行基线](qa-workspace-agent-m0-runtime-2026-09-26.md) |
| 数据库与多语言 | 隔离 PGlite：21 oracle、4 召回组、16 查询/计划；明确 SQL 探针与现有 RPC，见[数据库基线](qa-workspace-agent-m0-database-2026-09-26.md) |
| 标题预算 | 16 次真实短请求；14 规则检查通过 / 2 英文语言失败，冻结保留临时标题、不自动重试，见[标题基线](qa-workspace-agent-m0-titles-2026-09-26.md) |

后续按依赖推进：M1 统一云端资料访问与来源保存、M2 会话管理与持久草稿可并行；之后完成 M3 选文输入。M4–M6 继续沿原计划推进。M0 本轮交付状态为“契约冻结 + 验证工具完成”，尚无 M1–M7 新运行能力、测试部署或正式部署。
