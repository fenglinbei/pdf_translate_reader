...
- 当前账号系统支持登录、账号内 PDF 云端库、多设备打开同一份 PDF。
- 当前同步范围包括批注、翻译缓存、paper context、悬浮翻译卡片、用户设置和 API logs。
- 暂不做 OCR、全文预翻译、复杂笔记系统和离线 mutation queue。
...
- 设置变更后下一次翻译生效。
- 拖动选词超过最大词数时按配置裁剪，默认最多 `128` words，配置输入不可超过项目配置上限 `512` words。
- 主阅读界面不被设置项占据。
- 刷新页面后设置保留。
