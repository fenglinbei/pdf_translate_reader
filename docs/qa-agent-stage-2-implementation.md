...
日期：2026-09-24。QA 版本：`0.2.0-alpha.1`；应用版本：`0.1.0`。

代码已实现，首轮全项目检查通过，供应商真实合成调用已通过。**独立测试库尚未应用迁移，本地现有 QA 服务仍保留旧制品，生产未部署；真实论文的人工验收待进行。** 本文会明确区分合成工程验证与人工质量结论。
...
Playwright 已用真实 QA/PDF 组件与浏览器内合成 API 记录验证：无 QA 索引仍可提问、刷新恢复历史引用、关键句所在行高亮、跨页起点与后续页选择、未选父编号不可点击。PDF 为两页合成资料；这不代表真实测试库的登录/落库全链路已通过。截图保留于本机 `output/playwright/qa-p2/`。
...
模型能力按 `QA_DOCUMENT_MODELS` 开放，默认先开放 DeepSeek Flash/V4 Pro；Qwen 需显式加入并配置官方区域接口。GLM/Kimi 原生工具路径未开放。上述小样本不代表所有推理档均完成人工质量验收。
...
当前缺少测试数据库直连/管理入口，API 字段探测确认迁移未应用，因此尚未切换旧 QA 进程。不能把新版部署到未迁移的库后宣称可以验收。
