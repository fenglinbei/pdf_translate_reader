...
日期：2026-09-24。状态：**设计交付 / 未实施 / 未部署 / 未验收**。
...
| `finish_reading` | mode=grounded/direct/insufficient、evidenceIds、简短 answerOutline | 校验后的答案准备结果 | 证据足够、无需论文的交流，或确认当前证据不足 |
...
- 搜索预览也有准确来源范围。模型可引用实际返回的原文范围，但回答涉及更大上下文时必须继续阅读。
...
| 供应商 | 接入计划 | 本轮证据边界 |
| --- | --- | --- |
| DeepSeek | 首个闭环候选；模型 ID 沿用目录，支持现有选择，不擅自换默认 | 官方工具及 reasoning 契约已核对，原生循环未做真实调用验收 |
...
阅读结果以 `documentVersion + page/line range + viewVersion` 形成稳定 evidenceKey。
...
| `user_qa_citations` | 增加 source_kind、source_version、evidence_key、source_record_id、source_locator；chunk_id 允许新来源分支为空 | 旧行默认 indexed_chunk，旧来源仍强制真实 chunk 外键；新来源必须有已授权解析记录与定位 |
...
| P2-D 引用和界面 | 兼容迁移、新旧证据/引用类型、坐标、历史展示与 Ask readiness | 独立数据库验证新旧数据、隔离、删除/重解析；QA 前端可人工使用无索引文档 |
