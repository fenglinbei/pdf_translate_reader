...
日期：2026-09-24。状态：**实现、独立测试库迁移与本地部署已完成，用户已确认总体验收通过；生产未部署**。
...
以下保留批准的设计与验收标准；工具、字段、预算和迁移的实际落地见实施记录。
...
已使用 `QA_AGENT_RUNTIME=legacy-json-v1|document-tools-v1` 选择执行器；环境未设置时仍为 legacy，本地独立测试服务已显式开启新路径。
该开关不修改现有 executionMode 的公共含义，不自动触发索引或生产切流。
新路径的语义检索工具默认不注册。若评测说明有必要，再单独设计显式启用策略，不把它变成隐藏依赖。

原生工具功能已在 `0.2.0-alpha.1` 实现；本地交付版本为 `0.2.0-alpha.4`，默认使用 DeepSeek V4.1 Flash；已补齐就绪提示、公式资料/提示词和引用点击响应，后两项修复的人工复验待进行。
测试库迁移和本地 QA 重启已完成；未打正式发布标签，也未迁移或部署生产。
