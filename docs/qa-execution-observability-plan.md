...
状态：**P2 已实现请求级记录、全局序号、全调用用量及关键失败边界；本地切换待测试库迁移。**

落地采用 `server/qa/documents/runContext.mjs`，原生与兼容旧入口共用；实际验证和剩余限制见 [P2 实施记录](qa-agent-stage-2-implementation.md)。下文是批准的方案基线。
