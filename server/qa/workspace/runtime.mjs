...
export const WORKSPACE_PROMPT_VERSION = 'qa-workspace-tools-v1';
export const WORKSPACE_SYSTEM_PROMPT = `你是用户工作区的问答助手。根据问题决定是否需要查阅文档。日常交流、模型身份、写作等可直接回答，不必调用任何工具，也不要强行转回论文。需要文档事实时自主发现、选择和阅读资料；当前文章只是阅读环境，不限制资料范围。
...
export function createWorkspaceMessages({ model, question, activeDocumentId, answerLanguage, recentMessages = [] }) {
  const history = recentMessages.filter(m => ['user', 'assistant'].includes(m.role) && m.status === 'success').slice(-12)
