import { requireUserDocument, createOrReuseQaThread, listQaMessagesForThread, insertQaMessage, updateQaMessage, insertQaCitations, deleteQaMessage } from '../../supabase/qa.mjs';
...
export async function handleWorkspaceStream(request, response, user, body, dependencies = {}) {
  ...
  try {
    ...
    context = makeContext({ userId: user.id, userDocumentId: body.activeDocumentId, messageId: assistant.id, threadId: thread.id, model: body.model, emit, runtimeVersion: DOCUMENT_RUNTIME_VERSION, promptVersion: DOCUMENT_PROMPT_VERSION, duplicateObservations: false, recordToolTrace: true });
    ...
    const workspace = dependencies.workspace ?? createWorkspaceTools({ userId: user.id, activeDocumentId: body.activeDocumentId, signal });
    const result = await runWorkspaceAgent({ adapter, model: body.model, question: userMessage.content, userId: user.id,
    ...
    snapshot = { scope, activeCloudDocumentId: body.activeDocumentId,
      ...
      retrieverVersion: DOCUMENT_RUNTIME_VERSION, evidence: verified.citations.map(toEvidenceSnapshot),
