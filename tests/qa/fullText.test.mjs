...
async function mockFetch(input, init) {
  ...
  if (fixture.route && url.origin === "https://model-fixture.invalid") {
    ...
    assert(body.messages.some((message) => message.content.includes(FULL_MMD)), "The answer model must receive the full paper.");
    return new Response([
...
it("completes the global route with full text, a saved answer and no retrieval fallback", async () => {
  ...
  assert.equal(fixture.logs[0].payload.longContext, true);
});
